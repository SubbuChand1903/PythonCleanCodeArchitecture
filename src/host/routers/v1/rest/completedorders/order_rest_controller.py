...
completed_order_router = APIRouter(prefix="/rest/completed", tags=["Completed Orders"])

@completed_order_router.get("/test")
@inject
async def test():
...
@completed_order_router.get("/search", response_model=Optional[List[CompletedOrderDTO]])
@inject
async def search_orders(
    ...
    order_service: SearchOrdersQueryService = Depends(
        Provide[Container.search_order_service]
    )
):
    try:

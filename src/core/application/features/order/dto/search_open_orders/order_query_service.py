...
from src.core.application.features.order.dto.search_open_orders.order_mappings import OrderMappings
...
class SearchOrdersQueryService:
    ...
    async def search_orders(self, search_string: str, user_id: int) -> List[OpenOrderDTO]:
        try:
            orders: List[OpenOrder] = await self.order_repository.search_orders(search_string, user_id)
            result: List[OpenOrderDTO] = OrderMappings.model_list_to_dto_list(orders)
            return result
